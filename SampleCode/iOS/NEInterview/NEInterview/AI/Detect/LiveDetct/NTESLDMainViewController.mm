...
- (void)startLiveDetect {
    ...
    [self.detector startLiveDetectWithBusinessID:@"" actionsHandler:^(NSDictionary * _Nonnull params) {
...
//            [self.navigationController pushViewController:vc animated:YES];
        ...
        case NTESLDOnlineCheckTimeout:
            [self.view makeToast:@"云端检测结果请求超时"];
            [self.navigationController popToRootViewControllerAnimated:YES];
...
//            [self.navigationController pushViewController:vc animated:YES];
        ...
        case NTESLDOnlineUploadFailure:
            [self.view makeToast:@"云端检测上传图片失败"];
            [self.navigationController popToRootViewControllerAnimated:YES];
