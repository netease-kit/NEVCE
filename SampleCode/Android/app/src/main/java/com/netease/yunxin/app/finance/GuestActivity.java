...
import com.netease.yunxin.app.finance.utils.NotificationUtils;
import com.netease.yunxin.app.finance.utils.ToastUtils;
...
//            NavUtils.startMeetingActivity(GuestActivity.this, roomId, guestNameET.getText().toString().trim());
    ...
    private void fetchAuthInfo() {
        AppNetApi.getAccountInfo(new HttpHelper.HttpResultCallback<AccountResponseModel>() {
            ...
            public void onSuccess(AccountResponseModel result) {
                ...
                if (result != null) {
                    AuthManager.getInstance().setAccountId(result.accountId);
                    AuthManager.getInstance().setAccountToken(result.accountToken);
    ...
    private void callWithCode() {
        ...
        SelfInfo selfInfo = new SelfInfo(guestNameET.getText().toString().trim(), selectBusinessModel.getCode(), selectBusinessModel.getDesc(),
                                         vipSwitch.isChecked());
